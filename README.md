...
[功能特性](#-功能特性) • [真实截图](#-界面预览) • [快速开始](#-快速开始) • [工作原理](#-工作原理) • [项目结构](#-项目结构) • [FAQ](#-faq) • [路线图](#-路线图)
...
> 截图来自 repo 根目录的 [`demo.html`](demo.html)（离线静态版）以及 `./bin/larry-server` 起服后打开的实时 Web UI。

![Larry Dashboard Preview](https://github.com/larryworkerone/larry/raw/main/demo.html)

| 实时大盘 | 指标卡片 | 互通延迟矩阵 |
|---|---|---|
| WebSocket 2s 刷新 | CPU/内存/网络曲线 | `from_id → to_id` 延迟 ms |
...
```
┌───────────────┐   WebSocket /api/agent    ┌───────────────────────┐
│  larry-agent  │ ────────────────────────► │ sudo? no: larry-server │
│  (metrics+  ) │                           │  · registry agents.json │
│   probe :36510│◄── mesh_ping ──────────   │  · store (64 采样环形) │
└───────────────┘   └──→ 结果 mesh_result─►  │  · hub (agent+browser) │
┌───────────────┐                           │  · mesh scheduler      │
│  larry-agent  │ ─────────────────────────► │                        │
└───────────────┘                           └───────────┬────────────┘
                                                        │  WS /ws (2s snapshot)
...
- [x] CPU / 内存 / SWAP / 磁盘 / 网络吞吐 / Load / 连接 / 进程 采集
